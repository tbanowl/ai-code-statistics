...
- 支持 SQLAlchemyJobStore 持久化任务调度状态
- 配置 `scheduler.jobstore.type: sqlalchemy` 启用持久化
- 持久化表：`apscheduler_jobs`（存储任务定义和下次运行时间）
- 业务执行记录继续使用 `task_executions` 表
...
- `PUT /worker/notes/update` - 创建/更新 notes（最后写入胜出）
- `DELETE /worker/notes/delete` - 删除 notes
- `POST /worker/notes/sync/info` - 获取同步信息

**环境变量:**
- `GIT_AI_NOTES_API_KEY` - Notes 服务 API 密钥（可选）
