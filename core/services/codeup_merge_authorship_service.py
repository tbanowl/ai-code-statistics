...
class CodeupMergeAuthorshipService:
    def __init__(
        ...
        self.calculator = MergeAuthorshipCalculator()
    ...
    def _process_task(self, task) -> dict[str, Any]:
        ...
        merge_base_sha = self._merge_base(repo_path, task)
        ...
        notes_data = [
            self._note_payload(
                ...
                branch=task.target_branch if commit_sha == task.merge_commit_sha else task.source_branch,
                content=content,
            )
            for commit_sha in related_shas
        ]
        return self.notes_service.batch_push_notes(task.repo_url, notes_data)
