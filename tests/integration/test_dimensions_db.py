...
def test_find_daily_aggregation_affected_dates_uses_sql_exists_for_authorship_notes(
    setup_dbs,
):
    metrics_db, stats_db = setup_dbs
    raw_id = metrics_db.save_metrics_raw(1, 3, "{}", 1710000000)
    ...
    event.listen(stats_db.engine, "before_cursor_execute", capture_sql)
    try:
        result = stats_db.find_daily_aggregation_affected_dates(
            repo_url="example.com/org/repo",
            require_authorship_notes=True,
        ...
        event.remove(stats_db.engine, "before_cursor_execute", capture_sql)

    assert result == {"commit_dates": [20240309], "last_id": "c002"}
    emitted_sql = "\n".join(statements).lower()
    assert "exists" in emitted_sql
    assert "authorship_notes.commit_sha in" not in emitted_sql
