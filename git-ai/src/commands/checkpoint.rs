...
/// Returns the concurrency limit for file I/O operations (file state saving).
/// On slow disks (GIT_AI_SLOW_VM=1), this reduces from 8 to 2 to minimize
/// random I/O contention. Can be overridden via GIT_AI_FILE_IO_CONCURRENCY.
fn file_io_concurrency() -> usize {
    if let Ok(v) = std::env::var("GIT_AI_FILE_IO_CONCURRENCY").and_then(|v| v.parse::<usize>()) {
        return v;
    }
...
/// Returns the concurrency limit for attribution tasks.
/// On slow disks (GIT_AI_SLOW_VM=1), this reduces from 30 to 4.
/// Can be overridden via GIT_AI_ATTR_CONCURRENCY.
fn attr_concurrency() -> usize {
    if let Ok(v) = std::env::var("GIT_AI_ATTR_CONCURRENCY").and_then(|v| v.parse::<usize>()) {
        return v;
    }
    if std::env::var_os("GIT_AI_SLOW_VM").is_some() {
        return 4;
...
fn save_current_file_states(
    ...
    // Process files concurrently with a semaphore limiting concurrent reads.
    // On slow disks (GIT_AI_SLOW_VM=1), concurrency is reduced to 2 to avoid
    // random I/O amplification. Can be overridden via GIT_AI_FILE_IO_CONCURRENCY.
    let file_content_hashes = smol::block_on(async {
        let semaphore = Arc::new(smol::lock::Semaphore::new(file_io_concurrency()));
        ...
        let futures = files.iter().map(|file_path| {
            ...
            async move {
                ...
                // Write content to blob file, but only if it doesn't already exist.
                // This avoids redundant I/O on repeated edits of the same file content.
...
async fn get_checkpoint_entries(
    ...
    // Limit concurrent attribution tasks. On slow disks (GIT_AI_SLOW_VM=1), this is
    // reduced to 4. Can be overridden via GIT_AI_ATTR_CONCURRENCY.
    let semaphore = Arc::new(smol::lock::Semaphore::new(attr_concurrency()));
